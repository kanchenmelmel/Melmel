...
//add custom post type for items
add_action( 'init', 'my_custom_post_type_item', 25 );
function my_custom_post_type_item() {
    global $wp_post_types;
    ...
    if( isset( $wp_post_types[ $post_type_name ] ) ) {
        ...
        $wp_post_types[$post_type_name]->rest_base = 'discounts';
...
add_filter( 'rest_prepare_item', 'add_meta_to_json', 10, 3 );
...
$latitude = get_post_meta( $post->ID, 'jv_item_lat', true );
if(empty($latitude)){
    $latitude = '';
}

$longtitude = get_post_meta( $post->ID, 'jv_item_lng', true );
if(empty($longtitude)){
    $longtitude = '';
}
...
add_filter( 'rest_prepare_item', 'my_rest_prepare_post', 10, 3 );




function comment_raw_content($data, $post, $request) {
      $data->data['content']['raw'] = $post->comment_content;
      return $data;
}


add_filter( 'rest_prepare_comment', 'comment_raw_content', 10, 3 );
